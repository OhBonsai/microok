...
| 2026-07-27 | packages/app/src/app.tsx | Phase 3：接入 notes 功能。现状：NotesCommand 全局挂载于 SharedProviders；顶层路由 /notes/:dir（NotesRoute，复用 provider 链但不含 legacy chrome）；实现在新文件 src/pages/notes*.tsx |
